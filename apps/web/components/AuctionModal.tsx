        ...
        <Dialog open={true} maxWidth="sm" fullWidth disableEscapeKeyDown>
            <DialogTitle sx={{ textAlign: 'center', bgcolor: 'warning.light', color: 'warning.contrastText' }}>
                <Stack direction="row" alignItems="center" justifyContent="center" spacing={1}>
                    <GavelIcon />
                    <Typography variant="h5" fontWeight="bold">AUCTION IN PROGRESS</Typography>
                </Stack>
            </DialogTitle>
            <DialogContent>
                <Box textAlign="center" py={2}>
                    <Typography variant="h6" gutterBottom>{property?.name || 'Unknown Property'}</Typography>
                    {property?.group_color && (
                        <Typography variant="body2" color="text.secondary" gutterBottom>
                            Group: {property.group_color}
                        </Typography>
                    )}

                    <Box my={3} p={2} bgcolor="background.defaut" borderRadius={2} border="1px solid" borderColor="divider">
                        <Typography variant="subtitle2" color="text.secondary">Current Highest Bid</Typography>
                    ...
                    <Stack spacing={2}>
                        ...
                        <Button
                            ...
                            disabled={placeBid.isPending}
                        >
                            BID ${(currentBid + 10).toLocaleString()}
                        </Button>
                    </Stack>
                </Box>
            </DialogContent>
